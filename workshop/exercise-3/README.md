...
1.  Clone the Guestbook app into the `workshop` directory.
    ...
    ```shell
    git clone https://github.com/rvennam/guestbook
    ```
...
1.  Create a project called "guestbook"
    ``` shell
    oc new-project guestbook
    ```

2.  Annotate the guestbook namespace to enable automatic sidecar injection:
    
    ``` shell
    oc label namespace guestbook istio-injection=enabled
    ```
...
1. Inject the Istio Envoy sidecar into the guestbook pods, and deploy the Guestbook app on to the Kubernetes cluster. Deploy both the v1 and v2 versions of the app:
    ...
    ```shell
    oc apply -f ../v1/guestbook-deployment.yaml
